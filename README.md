...
**A. Create the `content-edits` branch in the repo** (Pages CMS can only target a branch that already exists). One-liner from a local clone:
...
A copy-pasteable starter template lives at [`docs/event-template.md`](docs/event-template.md) — outside `src/content/events/` on purpose so Pages CMS doesn't try to render it as an event.

**Step by step on github.com:**

1. **Open the template in a new tab** — go to `docs/event-template.md`, click **Raw**. Keep this tab open.
